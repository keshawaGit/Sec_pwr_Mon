...
INA219_WE usb1 = INA219_WE(I2C_ADDRESS_usb1);

// Add your MQTT Broker IP address, example:
const char* ssid = "BIXBY";//"BIXBY";
const char* password = "passwordIS@321";//"passwordIS@321";
...
void device_data(){
 StaticJsonDocument<200> doc;
   ...
   // Serial.println("Sending message to MQTT topic..");
    ...
    if (client.publish("dev/data", output) == true) {
...
void bus_data(){
  ...
  shuntVoltage_usb1 = usb1.getShuntVoltage_mV();
  busVoltage_usb1 = usb1.getBusVoltage_V();
  current_usb1 = usb1.getCurrent_mA();
  power_usb1 = usb1.getBusPower();
  loadVoltage_usb1  = busVoltage_usb1 + (shuntVoltage_usb1/1000);
  //ina219_overflow_usb1 = usb1.getOverflow();
 ...
 StaticJsonDocument<160> doc;
   ...
   // Serial.println("Sending message to MQTT topic..");
    ...
    if (client.publish("esp32/source", output) == true) {
      Serial.println("Success sending message");
    } else {
      Serial.println("Error sending message");
    }  
...
void loop() {
  ...
  /*
  char output[] ="test";
    if (client.publish("bus/data", output) == true) {
      Serial.println("Success sending message");
    } else {
      Serial.println("Error sending message");
    } 
